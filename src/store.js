    ...
    return createStore((set, get) => {
        ...
        return {
            ...
            renameNode: (oldId, nextId) => {
                if (!nextId || nextId === oldId || get().structure.nodes.some((n) => n.id === nextId))
                    return;
                const nodes = get().structure.nodes.map((n) => n.id === oldId ? { id: nextId, type: typeOf(nextId) } : n);
                ...
                const positions = { ...get().positions };
                if (positions[oldId]) {
                    positions[nextId] = positions[oldId];
